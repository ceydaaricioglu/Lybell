...
import { saveTaskToSupabase, fetchTasksFromSupabase, getMockCategories, filterRecurringTasks, syncTaskToGoogleCalendar, deleteTaskFromSupabase } from '@cursor-deneme/shared';
...
export default function Home() {
  ...
  /** Yeni görev modalı (web: Stitch tasarımı) */
  ...
  const handleSaveTaskFromModal = async (task: TimelineTask) => {
    if (!isPro && tasks.length >= FREE_MAX_TASKS) {
    ...
    if (isPro && savedId) {
      const taskWithId = { ...task, id: savedId };
      if (task.syncToGoogle) {
        await syncTaskToGoogleCalendar(userId, taskWithId, 'create', savedId);
      }
    }
    setShowNewTaskModal(false);
    loadTasks();
      ...
      return (
        <TasksView
          ...
          onDeleteTask={async (taskId) => {
            await deleteTaskFromSupabase(userId, taskId);
            loadTasks();
          }}
          tasks={tasks}
          setTasks={setTasks}
  ...
  return (
    <LocaleProvider>
      <div className={`min-h-screen ${isDarkMode ? 'bg-[#0f0f0f]' : 'bg-white'}`}>
        <div className="flex min-h-screen">
          ...
          <main className={`flex-1 min-w-0 overflow-auto ${showSidebar ? 'md:ml-64' : ''}`}>
            {currentView === 'category' || currentView === 'calendar' || currentView === 'home' ? (
              renderCurrentView()
            ) : (
              <div className="w-full max-w-md md:max-w-4xl lg:max-w-5xl px-4 md:pl-0 md:pr-8 py-4 md:pt-0 md:pb-6 main-content-pad md:pb-8 md:ml-0 md:mr-auto">
                {renderCurrentView()}
              </div>
            )}
          </main>
