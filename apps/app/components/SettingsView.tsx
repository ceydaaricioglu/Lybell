...
export default function SettingsView({ userId, onLogout, onSessionLost, darkMode = false, onDarkModeChange, onOpenProfile, onOpenPro, isPro = false, onNavTabsChange, onShowOnboarding, onExitPro, onBack }: SettingsViewProps) {
  ...
  return (
    <div className={`min-h-screen pb-24 ${dark ? 'bg-[#0f0f0f]' : 'bg-[#f5f0ea]'}`}>
      ...
      <div className={`border-b px-4 sm:px-6 py-5 ${dark ? 'bg-[#0f0f0f] border-zinc-800' : 'bg-[#f5f0ea] border-stone-200'}`}>
        <div className="max-w-md mx-auto">
          {onBack && (
            <button
              type="button"
              onClick={onBack}
              className="mb-3 w-10 h-10 flex items-center justify-center rounded-xl hover:bg-white/10 transition-colors"
