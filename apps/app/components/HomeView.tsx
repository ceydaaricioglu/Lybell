...
import { getMockCategories, fetchTasksFromSupabase, filterRecurringTasks, DEFAULT_TAGS, getTagColorClasses, getTodayPomodoroCount, getWeekPomodoroCount, getPomodoroStreak, getWeekCompletedCount, getMyDayTaskIds, toggleMyDayTask, getCountdownLabel } from '@cursor-deneme/shared';
  ...
  if (loading) {
    return (
      <div className={`min-h-screen pb-24 ${dark ? 'bg-[#0f0f0f]' : 'bg-[#f5f0ea]'}`}>
        <div className="w-full max-w-md md:max-w-none mx-auto md:mx-0 px-4 md:px-0 pt-6 pb-4 min-w-0 overflow-x-hidden">
