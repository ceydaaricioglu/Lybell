...
export default function SettingsView({ userId, onLogout, onSessionLost, darkMode = false, onDarkModeChange, onOpenProfile, onOpenPro, isPro = false, onNavTabsChange, onShowOnboarding, onExitPro }: SettingsViewProps) {
  ...
  return (
    <div className={`min-h-screen pb-24 ${dark ? 'bg-[#0f0f0f]' : 'bg-[#f5f0ea]'}`}>
      {/* Header */}
      <div className={`border-b px-4 py-4 ${dark ? 'bg-[#0f0f0f] border-zinc-800' : 'bg-[#f5f0ea] border-stone-200'}`}>
        <div className="max-w-md mx-auto">
          <h1 className={`text-lg font-semibold ${dark ? 'text-white' : 'text-stone-900'}`}>{t('settings.title', locale)}</h1>
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
        ...
        <button
          ...
          className={`w-full rounded-xl p-4 text-left transition-all ${dark ? 'bg-zinc-900/60 border border-zinc-800 hover:bg-zinc-800/60' : 'bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] border border-stone-100 hover:shadow-md'}`}
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h2 className={`text-base font-bold ${dark ? 'text-white' : 'text-stone-900'}`}>
                {userDisplayTitle}
              </h2>
        ...
        {isPro ? (
          <div className={`w-full rounded-xl p-4 text-left ${dark ? 'bg-gradient-to-r from-amber-500/20 to-orange-500/20 border border-amber-500/40' : 'bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200'}`}>
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${dark ? 'bg-amber-500/30' : 'bg-amber-100'}`}>
                <span className="text-2xl">👑</span>
              </div>
              <div className="flex-1 min-w-0">
                <h2 className={`text-base font-bold ${dark ? 'text-amber-200' : 'text-amber-800'}`}>
          ...
          <button
            type="button"
            onClick={onOpenPro}
            className={`w-full rounded-xl p-4 text-left transition-all ${dark ? 'bg-gradient-to-r from-amber-500/20 to-orange-500/20 border border-amber-500/40 hover:from-amber-500/30 hover:to-orange-500/30' : 'bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 hover:shadow-md'}`}
          >
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${dark ? 'bg-amber-500/30' : 'bg-amber-100'}`}>
                <span className="text-2xl">👑</span>
        ...
        <div>
          ...
          <div className={`rounded-xl overflow-hidden divide-y ${dark ? 'bg-zinc-900/60 border border-zinc-800 divide-zinc-800' : 'bg-white border border-stone-100 divide-stone-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]'}`}>
            {onShowOnboarding && (
              <button
                type="button"
        ...
        <div>
          ...
          <div className={`rounded-xl overflow-hidden ${dark ? 'bg-zinc-900/60 border border-zinc-800' : 'bg-white border border-stone-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]'}`}>
            {templates.length === 0 ? (
        ...
        <div>
          ...
          <div className={`rounded-xl p-4 mb-4 ${dark ? 'bg-zinc-900/60 border border-zinc-800' : 'bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] border border-stone-100'}`}>
            <div className="flex items-center justify-between mb-4">
              <h4 className={`text-base font-bold ${dark ? 'text-white' : 'text-stone-900'}`}>🍅 Pomodoro</h4>
              <span className={`text-xs ${dark ? 'text-zinc-500' : 'text-stone-500'}`}>Toplam Odaklanma</span>
            </div>
            <div className="grid grid-cols-4 gap-3 mb-4">
              <div className="text-center">
                <div className={`text-xl font-bold mb-0.5 ${dark ? 'text-amber-400' : 'text-amber-600'}`}>{getTodayPomodoroCount(userId)}</div>
                <div className={`text-[10px] ${dark ? 'text-zinc-500' : 'text-stone-600'}`}>Bugün</div>
              </div>
              <div className="text-center">
                <div className={`text-xl font-bold mb-0.5 ${dark ? 'text-amber-400' : 'text-amber-600'}`}>{getWeekPomodoroCount(userId)}</div>
                <div className={`text-[10px] ${dark ? 'text-zinc-500' : 'text-stone-600'}`}>Bu Hafta</div>
              </div>
              <div className="text-center">
                <div className={`text-xl font-bold mb-0.5 ${dark ? 'text-amber-400' : 'text-amber-600'}`}>{getMonthPomodoroCount(userId)}</div>
                <div className={`text-[10px] ${dark ? 'text-zinc-500' : 'text-stone-600'}`}>Bu Ay</div>
              </div>
              <div className="text-center">
                <div className={`text-xl font-bold mb-0.5 ${dark ? 'text-amber-400' : 'text-amber-600'}`}>{getPomodoroStreak(userId)}</div>
